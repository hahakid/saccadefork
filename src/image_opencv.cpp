...
// ====================================================================
    ...
    image mat_to_image2(cv::Mat mat, cv::Mat mat2);
...
image load_image_cv2(char *filename, char *filename2, int channels_f1, int channels_f2)
{
    cv::Mat mat = load_image_mat(filename, 3);

    char* sub = (char *)malloc(16); //object = 11, tracking = 16
    sub = right(sub, filename, 15); //object = 10, tracking = 15

    char *base_file = "/media/lyq/data/kitti/training/lidars/";
    char *filename_1 = join1(base_file, sub);
    // std::cout<<"filename_1: "<<filename_1<<endl;
    // cv::Mat mat2 = load_image_mat_hsv(filename_1, channels_f2);
    cv::Mat mat2 = cv::imread(filename_1, 3);
    // std::cout<<"w: "<<mat2.cols<<" h:"<<mat2.rows<<endl;
    if (mat.empty() || mat2.empty()){
        return make_image(10, 10, channels_f1 + channels_f2);
    }
    free(filename_1);
    //return mat_to_image2(mat, mat2);
    return mat_to_image2(mat, mat2);
}
...
image mat_to_image2(cv::Mat mat, cv::Mat mat2)
{
    int w = mat.cols;
    int h = mat.rows;
    int c = mat.channels() + mat2.channels();
    image im = make_image(w, h, c);
    unsigned char *data = (unsigned char *)mat.data;
    ...
    for (int y = 0; y < h; ++y) {
        for (int k = 0; k < mat.channels(); ++k) {
            for (int x = 0; x < w; ++x) {
                ...
                im.data[k*w*h + y*w + x] = data[y*step + x*mat.channels() + k] / 255.0f;
                if(im.data[k*w*h + y*w + x] < 0) count1++;
            }
        }
        for (int k = mat.channels(); k < c; ++k){
            for (int x = 0; x < w; ++x){
                //im.data[k*w*h + y*w + x] = data2[y*step + x*mat.channels() + k - 3] / 255.0f;
                im.data[k*w*h + y*w + x] = data2[y*step + x*mat.channels() + k - 3] / 255.0f;
                if(im.data[k*w*h + y*w + x] < 0) count2++;
            }
        }
    }
...
image image_data_augmentation2(mat_cv* mat, char* filename, int w, int h,
    int pleft, int ptop, int swidth, int sheight, int flip,
    float dhue, float dsat, float dexp,
    int blur, int num_boxes, float *truth)
{
    ...
    try {
        cv::Mat img = *(cv::Mat *)mat;
        //mat_cv *src2;
        //src2 = load_image_mat_cv(filename2, 1);

        cv::Mat img2 = cv::imread(filename, 1);

        // crop
        cv::Rect src_rect(pleft, ptop, swidth, sheight);
        cv::Rect src_rect2(pleft, ptop, swidth, sheight);
        
        cv::Rect img_rect(cv::Point2i(0, 0), img.size());
        cv::Rect img_rect2(cv::Point2i(0, 0), img2.size());
        
        cv::Rect new_src_rect = src_rect & img_rect;
        cv::Rect new_src_rect2 = src_rect2 & img_rect2;


        cv::Rect dst_rect(cv::Point2i(std::max<int>(0, -pleft), std::max<int>(0, -ptop)), new_src_rect.size());
        cv::Rect dst_rect2(cv::Point2i(std::max<int>(0, -pleft), std::max<int>(0, -ptop)), new_src_rect2.size());
        ...
        if (src_rect.x == 0 && src_rect.y == 0 && src_rect.size() == img.size()) {
            cv::resize(img, sized, cv::Size(w, h), 0, 0, cv::INTER_LINEAR);
            cv::resize(img2, sized2, cv::Size(w, h), 0, 0, cv::INTER_LINEAR);
        }
        else {
            cv::Mat cropped(src_rect.size(), img.type());
            cv::Mat cropped2(src_rect2.size(), img2.type());

            //cropped.setTo(cv::Scalar::all(0));
            cropped.setTo(cv::mean(img));
            cropped2.setTo(cv::mean(img2));

            img(new_src_rect).copyTo(cropped(dst_rect));
            img2(new_src_rect2).copyTo(cropped2(dst_rect2));

            // resize
            cv::resize(cropped, sized, cv::Size(w, h), 0, 0, cv::INTER_LINEAR);
            cv::resize(cropped2, sized2, cv::Size(w, h), 0, 0, cv::INTER_LINEAR);
        ...
        if (flip) {
            ...
            cv::Mat cropped2;
            cv::flip(sized2, cropped2, 1);    // 0 - x-axis, 1 - y-axis, -1 - both axes (x & y)
            sized2 = cropped2.clone();
        }
        ...
        // Mat -> image
        // out = mat_to_image2(sized, sized);
        out = mat_to_image2(sized, sized2);
